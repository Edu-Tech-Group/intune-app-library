...
Try {
    ...
    #Create Printer Port
    $PortExist = Get-Printerport -Name "IP_10.0.142.33" -ErrorAction SilentlyContinue
    if (-not $PortExist) {
        Add-PrinterPort -name "IP_10.0.142.33" -PrinterHostAddress "10.0.142.33" -Confirm:$false -ErrorAction SilentlyContinue
    }

    #Add Printer
    Add-Printer -Name "ELEVESAR" -DriverName "Epson Universal Print Driver" -PortName "IP_10.0.142.33" -Confirm:$false
}
Catch {
