...
Try {
    ...
    #Create Printer Port
    $PortExist = Get-Printerport -Name "IP_10.0.250.1" -ErrorAction SilentlyContinue
    if (-not $PortExist) {
        Add-PrinterPort -name "IP_10.0.250.1" -PrinterHostAddress "10.0.250.1" -Confirm:$false -ErrorAction SilentlyContinue
    }

    #Add Printer
    Add-Printer -Name "PROFESSEUR" -DriverName "Epson Universal Print Driver" -PortName "IP_10.0.250.1" -Confirm:$false
}
Catch {
